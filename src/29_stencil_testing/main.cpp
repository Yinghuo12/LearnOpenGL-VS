...
int main(int argc, char *argv[]) {
  ...
  // shader
  Shader sceneShader("../src/29_stencil_testing/shader/scene_vert.glsl", "../src/29_stencil_testing/shader/scene_frag.glsl");
  Shader lightObjectShader("../src/29_stencil_testing/shader/light_object_vert.glsl", "../src/29_stencil_testing/shader/light_object_frag.glsl");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glm::vec3 lightPos = glm::vec3(lightPosition.x * glm::sin(glfwGetTime()) * 2.0, lightPosition.y, lightPosition.z);  // 平行移动光源
    ...
    // 正常绘制地板
    glStencilMask(0x00);  //禁用写入, 即在绘制地板时不会更新模板缓冲
    ...
    // 2.绘制盒子放大版本，然后禁用模板写入
    // -----------------------------------------------------------
    glStencilFunc(GL_NOTEQUAL, 1, 0xff);   // 只在模板缓冲值不为1的区域绘制,即边框
    ...
    sceneShader.setFloat("stenci", 0.0);     // 绘制完盒子记得清空边框颜色，防止下一帧绘制
