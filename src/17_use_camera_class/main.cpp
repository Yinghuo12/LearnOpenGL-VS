...
int main() {
  ...
  Shader ourShader("../src/17_use_camera_class/shader/vertex.glsl", "../src/17_use_camera_class/shader/fragment.glsl");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // model = glm::eulerAngleZ(glfwGetTime()) * glm::eulerAngleY(glfwGetTime()) * glm::eulerAngleX(glfwGetTime());  // 欧拉旋转

    glm::qua<float> qu = glm::qua<float>(glm::vec3(glfwGetTime(), glfwGetTime(), glfwGetTime()));    // 四元数旋转
