...
int main() {
    ...
    // 着色器
    Shader ourShader("../src/07_load_texture/shader/vertex.glsl", 
                        "../src/07_load_texture/shader/fragment.glsl");
    ...
    if(data){
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);   // 传输纹理数据，开辟显存
    ...
    /* 重要 
        ...
        GL_LINEAR:在单个mipmap上采用线性插值
        MIPMAP_LINEAR:在两层mipmap上采用线性插值
        MIPMAP_NEAREST:在两层mipmap上采用最邻近采样
    */
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);        //S轴/X轴重复纹理图像
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);        //T轴/Y轴重复纹理图像
