...
int main() {
  ...
  // ------------------------------
  // 注册事件回调函数
  glfwSetFramebufferSizeCallback(window, framebuffer_size_callback);   //窗口变化
  glfwSetCursorPosCallback(window, mouse_callback);                    //鼠标移动
  glfwSetMouseButtonCallback(window, mouse_button_calback);            //鼠标点击
  glfwSetScrollCallback(window, scroll_callback);                      //鼠标滚轮
  // ------------------------

  // shader
  Shader ourShader("../src/19_basic_lighting/shader/vertex.glsl", "../src/19_basic_lighting/shader/fragment.glsl");
  Shader lightObjectShader("../src/19_basic_lighting/shader/light_object_vert.glsl", "../src/19_basic_lighting/shader/light_object_frag.glsl");
  ...
  ourShader.setVec3("lightColor", glm::vec3(1.0f, 1.0f, 1.0f));  // 光照颜色 白色
  ourShader.setFloat("ambientStrength", 0.1);                    // 环境光强度
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // ----------------------
    // 设置参数
    float rotate = glfwGetTime() * 0.2f;  // 用于model矩阵
    ...
    ourShader.setVec3("viewPos", camera.Position);    // 设置观察者位置即相机位置
