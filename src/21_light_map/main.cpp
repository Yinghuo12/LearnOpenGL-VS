...
int main() {
  ...
  // shader
  Shader ourShader("../src/21_light_map/shader/vertex.glsl", "../src/21_light_map/shader/fragment.glsl");
  Shader lightObjectShader("../src/21_light_map/shader/light_object_vert.glsl", "../src/21_light_map/shader/light_object_frag.glsl");
  ...
  // 传递材质属性
  // ourShader.setVec3("material.ambient", 1.0f, 0.5f, 0.31f);     // 环境光颜色
  ourShader.setFloat("material.shininess", 64.0f);                 // 高光指数
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // mvp矩阵 ------
    float rotate = glfwGetTime() * 0.2f;  //用于计算模型矩阵
