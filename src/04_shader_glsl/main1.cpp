...
int main() {
    ...
    // 附加着色器代码
    glShaderSource(vertexShader, 1, &vertexShaderSource, NULL);   // 1代表1个字符串
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        //glUseProgram(shaderProgram2);          //注意：更新一个uniform之前必须先使用程序，因为要指定传入到哪个着色器中。当然这句可以省略，因为上面已经使用了程序2。
        glUniform4f(ourColorLocation, 0.3f, greenValue, 0.2f, 1.f);
