...
int main(int argc, char *argv[]) {
  ...
  // shader
  Shader ourShader("../src/25_spot_light/shader/vertex.glsl", "../src/25_spot_light/shader/fragment.glsl");
  Shader lightObjectShader("../src/25_spot_light/shader/light_object_vert.glsl", "../src/25_spot_light/shader/light_object_frag.glsl");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // ****************  本节核心 ******************
    ourShader.setVec3("light.position", camera.Position);                   // 光源位置(把摄像机的位置设置为手电筒（聚光灯）的位置)
