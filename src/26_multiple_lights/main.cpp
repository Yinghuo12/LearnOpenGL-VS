...
int main(int argc, char *argv[]) {
  ...
  glm::vec3 lightPosition = glm::vec3(1.0, 2.5, 2.0); // 光照位置(用于左右移动灯光)
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    ourShader.setVec3("directionLight.direction", lightPos); // 平行光源位置
    ...
    lightObjectShader.setVec3("lightColor", glm::vec3(1.0f, 1.0f, 1.0f));  //设置移动灯光颜色
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      lightObjectShader.setMat4("model", model);
      lightObjectShader.setVec3("lightColor", pointLightColors[i]);   // 点光源颜色
